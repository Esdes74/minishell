...
char    **check_redirection(char **arg, t_cmd *struc)
{
    ...
    while (arg[i]) // récupération des différentes redirection
    {
        if (arg[i][0] == '<')
        {
            ...
            {
                if (arg[i + 1] == NULL)
                    return (struc->status = 1, anihilation(arg), error(TOKEN, "0"), NULL);
                file = open(arg[i + 1], O_RDONLY);
                if (file == -1)
                    return (struc->status = 1, anihilation(arg), error(OPEN, "0"), NULL);
                else if (dup2(file, STDIN_FILENO) == -1)
                    return (struc->status = 1, anihilation(arg), close(file), error(DUP, "0"), NULL);
                close(file);
                i++;
            }
    ...
    if (struc->heredoc == 1)
        if (write_hd_to_pip(struc) == 1)
            return (struc->status = 1, anihilation(arg), NULL);
    i = 0;
    compt = 0;
